...
def walk(s, choose_one=False):
    ...
    if isinstance(s, list):
        res = [(os.path.join(*[pp[0] for pp in p]), [pp[1] for pp in p]) for p
               in m]
    elif isinstance(s, dict):
        ...
        res = None
        print("s should either be a list of dict. Aborting. ")
        exit(-1)
    return res
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description="Generating configs for experiments")
    parser.add_argument(
        "--dir",
        help="the path to the template of interest")
    parser.add_argument(
        "--template",
        help="the name for the template yaml",
        default="template.yaml")
    parser.add_argument(
        "--pattern",
        help="the name for the cfg yaml",
        default="cfg.yaml")
    parser.add_argument(
        "--results_dir",
        help="directory under which the configs and results will be put. ",
        default="results")
    ...
    for index, (a, b) in enumerate(lst):
        ...
        path_dir = os.path.join(out_dir, args.results_dir, a)
