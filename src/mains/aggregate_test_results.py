...
parser.add_argument('--result_filename', default='test_result.yaml')
...
for model_type in model_list:
    ...
    for voting_rule in voting_rule_list:
        ...
        for dataset_name in os.listdir(vote_dir):
            ...
            with open(f"{data_dir}/{args.result_filename}", 'r') as yaml_f:
        ...
        if total_samples > 0.0:
            ...
            with open(f"{vote_dir}/{args.result_filename}", 'w') as yaml_f:
                yaml.dump(agg_acc, yaml_f, default_flow_style=False)
                print(f"Saved to {vote_dir}/{args.result_filename}")
        else:
